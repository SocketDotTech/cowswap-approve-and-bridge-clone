...
contract BungeeApproveAndBridge is ApproveAndBridge {
    ...
    ISocketVerifier immutable socketVerifier;

    constructor(ISocketGateway socketGateway_, ISocketVerifier socketVerifier_) {
        socketGateway = socketGateway_;
        socketVerifier = socketVerifier_;
    }
        ...
        override
    {
        ...
        // perform a verifier call
        // validate the verifier output based on the inputs
        bool isVerified = _verifySocketGatewayCalldata(token, amount, receiver, toChainId, modifiedCalldata);
        if (!isVerified) revert InvalidInput();
    ...
    function _verifySocketGatewayCalldata(
        IERC20 token,
        uint256 amount,
        address receiver,
        uint256 toChainId,
        ...
        // 1. Extract routeId from the first 4 bytes of data
        if (data.length < 4) revert InvalidInput();
        uint32 routeId;
        bytes4 implFunctionSelector;
        assembly {
            routeId := shr(224, mload(add(data, 32))) // first 4 bytes into uint32
            implFunctionSelector := shr(224, mload(add(data, 36))) // next 4 bytes into bytes4
        }

        // 2. Prepare the SocketRequest struct
        ISocketVerifier.SocketRequest memory expectedSocketRequest = ISocketVerifier.SocketRequest({
            amount: amount,
            recipient: receiver,
            toChainId: toChainId,
            token: address(token),
            signature: implFunctionSelector
        });

        // 3. Call validateSocketRequest
        // this will revert if the request is not valid, so return false on error, else true
        try socketVerifier.validateSocketRequest(data, expectedSocketRequest) {
            return true;
        } catch {
            return false;
        }
